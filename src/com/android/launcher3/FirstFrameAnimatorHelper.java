...
/*
 * This is a helper class that listens to updates from the corresponding animation. For the first
 * two frames, it adjusts the current play time of the animation to prevent jank at the beginning of
 * the animation
 */
public class FirstFrameAnimatorHelper extends AnimatorListenerAdapter implements ValueAnimator.AnimatorUpdateListener {
    ...
    public void onAnimationUpdate(final ValueAnimator animation) {
        ...
        // don't adjust the animation in that case
            ...
            } else if (frameNum == 1 && currentTime < mStartTime + MAX_DELAY && !mAdjustedSecondFrameTime
                    && currentTime > mStartTime + IDEAL_FRAME_DURATION && currentPlayTime > IDEAL_FRAME_DURATION) {
    ...
    public void print(ValueAnimator animation) {
        ...
        Log.d("FirstFrameAnimatorHelper", sGlobalFrameCounter + "(" + (sGlobalFrameCounter - mStartFrame) + ") "
                + mTarget + " dirty? " + mTarget.isDirty() + " " + flatFraction + " " + this + " " + animation);
    }
}
